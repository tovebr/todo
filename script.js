...
todoSubmitInput.addEventListener("click", (e) => {
  ...
  if (todoHeaderInput.value) {
    ...
    if (todos.length === 0) {
      ...
      id = todos.map((todo) => todo.id).reduce((a, b) => (a > b ? a : b)) + 1;
    }
    ...
    printCard(todos[todos.length - 1]);
  }
...
function editTodo(element) {
  ...
  if (classes.includes("edit")) {
    todoHeaderInput.value = todos[index].todoHeader;
    todoDeadlineInput.value = todos[index].deadline;
  }
...
function todoDone(element) {
  element.querySelector(".card-header").classList.toggle("done");
  const index = todos.findIndex((todo) => {
    return todo.id === Number(element.id);
  });
...
function sortTodos(sortBy) {
  todos.sort((a, b) => b[sortBy] - a[sortBy]);
  reprintTodos();
}
